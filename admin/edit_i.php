...
<body>
		...
		<div class="container">
			...
			if (!isset($_GET['edit_iid']) || $_GET['edit_iid'] == NULL) {
			echo "<script>window.location = 'firstaid_insertI.php';</script>";
			} else {
				$postid = $_GET['edit_iid'];
				}
			
			?>	
				<?php
				if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		$i_name = mysqli_real_escape_string($db->link, $_POST['i_name']);
		$i_desc = mysqli_real_escape_string($db->link, $_POST['i_desc']);
		...
		?>    
			<h2>Update</h2>
			<button class="but success"><a href="firstaid_insertI.php">Inserted Data</a></button>
			<form action="#" method="post" enctype="multipart/form-data">	
			<?php 
			
			$query = "select * from firstaidi where id='$postid' order by id";
			...
			<input type="text" name="i_name" value="<?php echo $result['i_name']; ?>">
			...
			<label>FirstAid Desc</label>
              ...
              <textarea class="tinymce" name="i_desc" >
			  <?php echo $result['i_desc']; ?>
			  </textarea>
