...
<body>
		...
		<div class="container">
			...
			if (!isset($_GET['edit_jid']) || $_GET['edit_jid'] == NULL) {
			echo "<script>window.location = 'firstaid_insertJ.php';</script>";
			} else {
				$postid = $_GET['edit_jid'];
				}
			
			?>	
				<?php
				if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		$j_name = mysqli_real_escape_string($db->link, $_POST['j_name']);
		$j_desc = mysqli_real_escape_string($db->link, $_POST['j_desc']);
		...
		?>    
			<h2>Update</h2>
			<button class="but success"><a href="firstaid_insertJ.php">Inserted Data</a></button>
			<form action="#" method="post" enctype="multipart/form-data">	
			<?php 
			
			$query = "select * from firstaidj where id='$postid' order by id";
			...
			<input type="text" name="j_name" value="<?php echo $result['j_name']; ?>">
			...
			<label>FirstAid Desc</label>
              ...
              <textarea class="tinymce" name="j_desc" >
			  <?php echo $result['j_desc']; ?>
			  </textarea>
