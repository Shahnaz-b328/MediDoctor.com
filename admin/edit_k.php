...
<body>
		...
		<div class="container">
			...
			if (!isset($_GET['edit_kid']) || $_GET['edit_kid'] == NULL) {
			echo "<script>window.location = 'firstaid_insertK.php';</script>";
			} else {
				$postid = $_GET['edit_kid'];
				}
			
			?>	
				<?php
				if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		$k_name = mysqli_real_escape_string($db->link, $_POST['k_name']);
		$k_desc = mysqli_real_escape_string($db->link, $_POST['k_desc']);
		...
		?>    
			<h2>Update</h2>
			<button class="but success"><a href="firstaid_insertK.php">Inserted Data</a></button>
			<form action="#" method="post" enctype="multipart/form-data">	
			<?php 
			
			$query = "select * from firstaidk where id='$postid' order by id";
			...
			<input type="text" name="k_name" value="<?php echo $result['k_name']; ?>">
			...
			<label>FirstAid Desc</label>
              ...
              <textarea class="tinymce" name="k_desc" >
			  <?php echo $result['k_desc']; ?>
			  </textarea>
