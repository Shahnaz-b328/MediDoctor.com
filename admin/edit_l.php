...
<body>
		...
		<div class="container">
			...
			if (!isset($_GET['edit_lid']) || $_GET['edit_lid'] == NULL) {
			echo "<script>window.location = 'firstaid_insertL.php';</script>";
			} else {
				$postid = $_GET['edit_lid'];
				}
			
			?>	
				<?php
				if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		$l_name = mysqli_real_escape_string($db->link, $_POST['l_name']);
		$l_desc = mysqli_real_escape_string($db->link, $_POST['l_desc']);
		...
		?>    
			<h2>Update</h2>
			<button class="but success"><a href="firstaid_insertL.php">Inserted Data</a></button>
			<form action="#" method="post" enctype="multipart/form-data">	
			<?php 
			
			$query = "select * from firstaidl where id='$postid' order by id";
			...
			<input type="text" name="l_name" value="<?php echo $result['l_name']; ?>">
			...
			<label>FirstAid Desc</label>
              ...
              <textarea class="tinymce" name="l_desc" >
			  <?php echo $result['l_desc']; ?>
			  </textarea>
