...
<body>
		...
		<div class="container">
			...
			if (!isset($_GET['edit_oid']) || $_GET['edit_oid'] == NULL) {
			echo "<script>window.location = 'firstaid_insertO.php';</script>";
			} else {
				$postid = $_GET['edit_oid'];
				}
			
			?>	
				<?php
				if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		$o_name = mysqli_real_escape_string($db->link, $_POST['o_name']);
		$o_desc = mysqli_real_escape_string($db->link, $_POST['o_desc']);
		...
		?>    
			<h2>Update</h2>
			<button class="but success"><a href="firstaid_insertO.php">Inserted Data</a></button>
			<form action="#" method="post" enctype="multipart/form-data">	
			<?php 
			
			$query = "select * from firstaido where id='$postid' order by id";
			...
			<input type="text" name="o_name" value="<?php echo $result['o_name']; ?>">
			...
			<label>FirstAid Desc</label>
              ...
              <textarea class="tinymce" name="o_desc" >
			  <?php echo $result['o_desc']; ?>
			  </textarea>
