...
<body>
		...
		<div class="container">
			...
			if (!isset($_GET['edit_wid']) || $_GET['edit_wid'] == NULL) {
			echo "<script>window.location = 'firstaid_insertW.php';</script>";
			} else {
				$postid = $_GET['edit_wid'];
				}
			
			?>	
				<?php
				if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		$w_name = mysqli_real_escape_string($db->link, $_POST['w_name']);
		$w_desc = mysqli_real_escape_string($db->link, $_POST['w_desc']);
		...
		?>    
			<h2>Update</h2>
			<button class="but success"><a href="firstaid_insertW.php">Inserted Data</a></button>
			<form action="#" method="post" enctype="multipart/form-data">	
			<?php 
			
			$query = "select * from firstaidw where id='$postid' order by id";
			...
			<input type="text" name="w_name" value="<?php echo $result['w_name']; ?>">
			...
			<label>FirstAid Desc</label>
              ...
              <textarea class="tinymce" name="w_desc" >
			  <?php echo $result['w_desc']; ?>
			  </textarea>
