...
<body>
		...
		<div class="container">
			...
			if (!isset($_GET['edit_xid']) || $_GET['edit_xid'] == NULL) {
			echo "<script>window.location = 'firstaid_insertX.php';</script>";
			} else {
				$postid = $_GET['edit_xid'];
				}
			
			?>	
				<?php
				if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		$x_name = mysqli_real_escape_string($db->link, $_POST['x_name']);
		$x_desc = mysqli_real_escape_string($db->link, $_POST['x_desc']);
		...
		?>    
			<h2>Update</h2>
			<button class="but success"><a href="firstaid_insertX.php">Inserted Data</a></button>
			<form action="#" method="post" enctype="multipart/form-data">	
			<?php 
			
			$query = "select * from firstaidx where id='$postid' order by id";
			...
			<input type="text" name="x_name" value="<?php echo $result['x_name']; ?>">
			...
			<label>FirstAid Desc</label>
              ...
              <textarea class="tinymce" name="x_desc" >
			  <?php echo $result['x_desc']; ?>
			  </textarea>
